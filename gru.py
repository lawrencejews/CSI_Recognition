...
class CSIModelConfig:
    ...
    def build_model(self, n_unit_lstm=200, n_unit_atten=400,):
        ...
        x_tensor = tf.keras.layers.GRU(units=n_unit_lstm, return_sequences=True)(x_in)
...
if __name__ == "__main__":
    ...
    history = model.fit(
        ...
        callbacks=[
            tf.keras.callbacks.ModelCheckpoint('best_gru.hdf5',
                                                monitor='val_accuracy',
                                                save_best_only=True,
                                                save_weights_only=False)
            ])
    # load the best model
    model = cfg.load_model('best_gru.hdf5')
