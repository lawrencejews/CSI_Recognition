...
def extract_csi_by_label(raw_folder, label, labels, save=False, win_len=1000, thrshd=0.6, step=200):
    ...
    annot_csv_files = [os.path.basename(fname).replace(
        'input_', 'annotation_') for fname in input_csv_files]
    annot_csv_files = [os.path.join(raw_folder, fname)
                       for fname in annot_csv_files]
    feature = []
    index = 0
    for csi_file, label_file in zip(input_csv_files, annot_csv_files):
        ...
        if not os.path.exists(label_file):
            ...
            continue
        feature.append(merge_csi_label(csi_file, label_file,
                                       win_len=win_len, thrshd=thrshd, step=step))
        print('Finished {:.2f}% for Label {}'.format(
            index / len(input_csv_files) * 100, label))
...
def extract_csi(raw_folder, labels, save=False, win_len=1000, thrshd=0.6, step=200):
    ...
    ans = []
    for label in labels:
        feature_arr, label_arr = extract_csi_by_label(
            raw_folder, label, labels, save, win_len, thrshd, step)
...
class CSIModelConfig:
    ...
    def __init__(self, win_len=1000, step=200, thrshd=0.6, downsample=2):
        ...
        self._step = step
        self._thrshd = thrshd
        self._labels = ("bed", "fall", "pickup", "run",
    ...
    def preprocessing(self, raw_folder, save=False):
        ...
        """
        numpy_tuple = extract_csi(
            raw_folder, self._labels, save, self._win_len, self._thrshd, self._step)
    ...
    def load_csi_data_from_files(self, np_files):
        ...
        if len(np_files) != 7:
            raise ValueError(
                'There should be 7 numpy files for bed, fall, pickup, run, sitdown, standup, walk.')
    ...
    def build_model(self, n_unit_lstm=200, n_unit_atten=400,):
        ...
        pred = tf.keras.layers.Dense(
            len(self._labels), activation='softmax')(x_tensor)
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) != 2:
        print("Error! Correct Command: python3 csi_extractor.py Dataset_folder_path")
